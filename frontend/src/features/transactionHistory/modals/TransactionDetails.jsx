      ...
      <div className="bg-[#FDFDFD] w-full max-w-md rounded-xl shadow-lg sm:px-6 sm:py-4 p-4 relative animatefadeIn">
        ...
        <div className="space-y-2 text-sm sm:text-lg">
          <div className="flex justify-between">
            <p className="font-medium text-gray-700">Transaction Name :</p>
            <p className="text-gray-800">{transaction?.name}</p>
          </div>
          {/* ID */}
          <div className="flex justify-between">
            <p className="font-medium text-gray-700">Transaction ID :</p>
            <p className="text-gray-800">{transaction?.displayId}</p>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <p className="text-gray-800">{transaction?.phone}</p>
          </div>

          {/* Coin Balance */}
          <div className="flex justify-between">
            <p className="font-medium text-gray-700">Email :</p>
            <p className="text-gray-800">{transaction?.email}</p>
          </div>

          {/* Coin Spend */}
          <div className="flex justify-between">
            <p className="font-medium text-gray-700">WhatsApp :</p>
            <p className="text-gray-800">{transaction?.whatsapp}</p>
          </div>
          ...
          {/* <div className="flex justify-between">
            ...
            <span
              className={`px-3 py-1 text-sm block text-center ${
                Transaction.status === "active" && !Transaction.ban.isTemporary
                  ? "bg-linear-to-r from-[#79D49B] to-[#25C962]"
                  : "bg-[#FF929296] text-[#D22B20]"
              } text-[#005D23] rounded-full font-semibold`}
            >
              {Transaction.ban.isTemporary ? "Temp. ban" : Transaction.status}
            </span>
          </div> */}
          ...
          <div className="flex justify-between">
            ...
            <p className="text-gray-800">{transaction?.documentType}</p>
          </div>

          {/* Registration Time */}
          {/* <div className="flex justify-between">
            <p className="font-medium text-gray-700">Registration Time :</p>
            <p className="text-gray-800">{formatOnlyDate(Transaction?.createdAt)}</p>
          </div> */}
          ...
          {/* <div className="flex items-center justify-between gap-4 mx-5 pt-3">
            ...
            <div className="flex flex-col gap-1.5 items-center">
              <img
                src={Transaction?.documentFrontURL}
                alt="Document Front"
                className="w-32 h-23 rounded-sm object-contain blur-xs hover:blur-none cursor-pointer"
                loading="lazy"
                onClick={() =>
                  downloadImage(
                    Transaction?.documentFrontURL,
                    `${Transaction?.name}-front.jpg`,
                  )
                }
              />
            ...
            <div className="flex flex-col gap-1.5 items-center">
              <img
                src={Transaction?.documentBackURL}
                alt="Document Back"
                className="w-32 h-23 rounded-sm object-contain blur-xs hover:blur-none cursor-pointer"
                loading="lazy"
                onClick={() =>
                  downloadImage(
                    Transaction?.documentBackURL,
                    `${Transaction?.name}-back.jpg`,
                  )
                }
              />
